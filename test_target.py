...
from src.app.kalama.target.trivy_scanner import scan_image, validate_trivy_json
from src.app.kalama.target.victim_manager import (
...
class TrivyTests(unittest.TestCase):
    ...
    def test_invalid_json_command_failure_and_replace_failure_do_not_publish(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            path.write_text("old")
            with patch("src.app.kalama.target.trivy_scanner.os.replace", side_effect=OSError("boom")):
                with self.assertRaises(Step2OperationError) as caught:

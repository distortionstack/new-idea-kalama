...
from kalama.orchestrator import (
...
from kalama.state.store import (
...
class StateStoreTests(unittest.TestCase):
    ...
    def test_atomic_replace_failure_preserves_old_state(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("kalama.state.store.os.replace", side_effect=OSError("boom")):
    ...
    def test_serialization_failure_preserves_old_state(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            path = store.path_for(state.run_id)
            before = path.read_bytes()
            with patch("kalama.state.store.json.dumps", side_effect=TypeError("bad value")):
                with self.assertRaises(StateStoreError):
                    store.save(replace(state, status=RunStatus.RUNNING))
            self.assertEqual(path.read_bytes(), before)

...
from kalama.prioritizer.enrichment import (
...
from kalama.prioritizer.models import (
    CVSSCandidate, CVSSRecord, EPSSRecord, EvidenceState, KEVCatalogSnapshot,
    KEVState, VulnerabilityOccurrence,
)
from kalama.prioritizer.pipeline import (
    prioritize_trivy, serialize_artifact, write_artifact_atomic,
)
from kalama.prioritizer.exposure import exposure_from_facts
from kalama.prioritizer.scoring import rank_cves, score_cve
from kalama.prioritizer.trivy_parser import (
...
class ProviderTests(unittest.TestCase):
    ...
    def test_epss_transport_resolves_only_ipv4_without_global_socket_changes(self):
        ...
        with patch("kalama.prioritizer.enrichment.socket.getaddrinfo",
                   return_value=[(socket.AF_INET, socket.SOCK_STREAM, 6, "", ("192.0.2.1", 443))]) as lookup, \
             patch("kalama.prioritizer.enrichment.socket.socket",
...
class ScoringPipelineTests(unittest.TestCase):
    ...
    def test_parsing_does_not_mutate_input_and_realistic_fixture(self):
        ...
        fixture = Path(__file__).parent / "fixtures/trivy_realistic.json"
    ...
    def test_atomic_write_failure_leaves_existing_canonical_file(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            path.write_text("old")
            with patch("kalama.prioritizer.pipeline.os.replace", side_effect=OSError("boom")):

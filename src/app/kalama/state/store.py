...
def run_state_from_dict(data: Any, *, expected_run_id: str | None = None) -> RunState:
    ...
    remediation_basis = root.get("remediation_basis", "exploit-confirmed")
    if not isinstance(remediation_basis, str) or remediation_basis not in {
            "exploit-confirmed", "scanner-confirmed"}:
        raise ValueError("remediation_basis must be 'exploit-confirmed' or 'scanner-confirmed'")
